...
class PhyNetwork(object):
    ...
    def __init__(self,baseport=10000, numberOfNodesPerRing=4):
        self.__networkList = [[]]
        self.baseport=baseport
        self.numberOfNodesPerRing=numberOfNodesPerRing
        self.__phyMaster=PhyMaster.PhyMaster(self)
    ...
    def API_dumpPhyNetworkState(self):
        for thisRing in self.__networkList:
            for thisNode in thisRing:
                self.__debugOut.out("Node : Connection %s : ClientAddr %s : ListenInterfacePorts: %s : SendInterfaceConfig : %s " % (thisNode.connection, thisNode.clientAddr,thisNode.listenInterfacePorts, thisNode.sendInterfaceConfig))
